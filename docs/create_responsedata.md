...
- `expanded` 행의 값 변환은 코드북의 `value_code_map`을 그대로 따릅니다.
  ...
  - 주관식 시간 `expanded` → 오전/오후는 코드/라벨 변환하고, `시`, `분`은 숫자값을 그대로 적재
...
- 주관식 시간 `raw` 값은 원본의 시간 구성값을 보존하되, 24시간제 입력은 `responsedata_value.csv`에 `시|분`, `responsedata_label.csv`에 `H시 M분` 형식으로 생성합니다.
...
### 5-8. 주관식 시간
하루 중 특정 시간을 `시`와 `분`, 또는 `오전/오후`와 `시`와 `분`으로 나누어 직접 입력하는 문항입니다.
...
| 파일 | 값 형식 | 예시 |
|:---|:---|:---|
| `responsedata_value.csv` | 시간 구성값 (파이프 구분자) | `9\|30`, `1\|6\|30` |
| `responsedata_label.csv` | 사람이 읽는 시간 표현 | `9시 30분`, `오전 6시 30분` |

#### 오전/오후, 시/분 확장행 (`data_column_role = expanded`)
오전/오후, 시/분 `expanded` 열은 실제 계산에 사용하는 구성값 컬럼입니다.

| 파일 | 값 형식 | 예시 |
|:---|:---|:---|
| `responsedata_value.csv` | 오전/오후 코드 또는 `시`/`분` 숫자값 | `1`, `9`, `30` |
| `responsedata_label.csv` | 오전/오후 라벨 또는 `시`/`분` 숫자값 | `오전`, `9`, `30` |
...
- 총 분 단위 `derived`는 24시간제 입력이면 `시 * 60 + 분`, 12시간제 입력이면 `ampm_to_24h(오전/오후, 시) * 60 + 분`으로 계산합니다.
  - 예: `9|30` → `570`
